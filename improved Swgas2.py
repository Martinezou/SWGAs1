...
def calculate_cost(a, b, c):
    """计算每个零部件的成本
       ...
       """
    # 零部件的工序加工费用
    cost = [[]for i in xrange(len(a))]  # 50个零部件的加工费用
    element = []
...
# 繁殖代数10， 20， 30， 40， 50， 100， 200， 300， 400， 500， 700， 1000
    for i in xrange(iterations):
        generation += 1
        list_neighbours = small_world_population(pop_size, 6, 0.8)
        best_neighbours = dominate_sort(list_neighbours, process_code_population1, machine_code_population1, 0.4)
        ...
        for i in xrange(len(process_code_population)):
            two_objective_list = calculate_two_objective(process_code_population[i], machine_code_population[i])
            result.append(two_objective_list)
        result = simple(result)
        print generation
        # print result
        best_result = dominated_sort1(simple(result))
        print best_result
        generation += 1
