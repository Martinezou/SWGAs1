...
def calculate_cost(a, b, c):
    """计算每个零部件的成本
       ...
       """
    # 零部件的工序加工费用
    cost = [[]for i in xrange(len(a))]  # 20个零部件的加工费用
    element = []
...
def small_world_population(a, b, c):
    ...
    #plt.show()
    list_neighbour = [[] for i in range(pop_size)]  # 节点邻居
    for i in xrange(pop_size):
        list_neighbour[i].extend(ER.neighbors(i))
    return list_neighbour


#list_neighbours = small_world_population(100, 4, 0.3)  # 每个节点的相邻节点
...
# 进化代数
        for i in range(iterations):
            ...
            new_process_code_population = new_code_population[0]
            new_machine_code_population = new_code_population[1]
            new_code_population = mutation(new_process_code_population, new_machine_code_population, 0.5)
            process_code_population1 = new_code_population[0]
            machine_code_population1 = new_code_population[1]
            trait1 = dictionary(process_code_population, machine_code_population, pop_size)
            trait2 = dictionary(process_code_population1, machine_code_population1, pop_size)
